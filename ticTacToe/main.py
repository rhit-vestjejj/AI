...
pygame.display.set_caption('Play LeBron in Tic Tac Toe')
screen.fill(BLACK)
...
def minimax(minimax_board, depth, is_maximizing):
    ...
    # Checks what move is best and trying it out
    if is_maximizing:
        best_score = -1000
        for row in range(BOARD_ROW):
            for col in range(BOARD_COL):
...
def best_move():
    if is_board_full():  # Add check to see if the board is full
        return False
    ...
    if move != (-1, -1):  # Fix condition to check for a valid move
